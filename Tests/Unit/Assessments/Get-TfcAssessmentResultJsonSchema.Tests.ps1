...
Describe 'Get-TfcAssessmentResultJsonSchema' {
    BeforeEach { Mock Invoke-TfcApi -ModuleName TerraformCloud }
    Context 'Parameter Validation' {
        It 'Should have AssessmentResultId parameter as mandatory' {
            (Get-Command Get-TfcAssessmentResultJsonSchema).Parameters['AssessmentResultId'].Attributes.Mandatory | Should -Be $true
        }
    }
    Context 'API Interaction' {
        It 'Should call correct API endpoint' {
            ...
            Should -Invoke Invoke-TfcApi -ModuleName TerraformCloud -Times 1 -ParameterFilter {
                $Uri -eq '/assessment-results/asmtresult-abc123/json-schema'
            }
        }
    }
}
