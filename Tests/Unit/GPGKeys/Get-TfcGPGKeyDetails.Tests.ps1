...
Describe 'Get-TfcGPGKeyDetails' {
    ...
    Context 'Parameter Validation' {
        It 'Should have RegistryName parameter as mandatory' {
            $command = Get-Command Get-TfcGPGKeyDetails
    ...
    Context 'API Interaction' {
        It 'Should call correct API endpoint with registry prefix' {
            Get-TfcGPGKeyDetails -RegistryName 'private' -Namespace 'test-namespace' -KeyId 'key-123'
            Should -Invoke Invoke-TfcApi -ModuleName TerraformCloud -Times 1 -ParameterFilter {
                $Uri -like '*/api/registry/private/v2/gpg-keys/test-namespace/key-123*'
            }
        }
    }
    ...
    Context 'Verbose Output' {
        It 'Should write verbose message' {
            $verboseOutput = Get-TfcGPGKeyDetails -RegistryName 'private' -Namespace 'test-namespace' -KeyId 'key-123' -Verbose 4>&1
