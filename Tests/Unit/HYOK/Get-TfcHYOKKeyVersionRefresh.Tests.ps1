...
Describe 'Get-TfcHYOKKeyVersionRefresh' {
    BeforeEach {
    ...
    Context 'Parameter Validation' {
        It 'Should have ConfigurationId parameter as mandatory' {
            $command = Get-Command Get-TfcHYOKKeyVersionRefresh
    ...
    Context 'API Interaction' {
        It 'Should call correct API endpoint' {
            ...
            Should -Invoke Invoke-TfcApi -ModuleName TerraformCloud -Times 1 -ParameterFilter {
                $Uri -like '/hyok-configurations/hyok-abc123/key-versions*'
            }
        }
    }
    ...
    Context 'Verbose Output' {
        It 'Should write verbose message' {
            $verboseOutput = Get-TfcHYOKKeyVersionRefresh -ConfigurationId 'hyok-abc123' -Verbose 4>&1
