...
Describe 'Test-TfcHYOKConfiguration' {
    BeforeEach {
    ...
    Context 'Parameter Validation' {
        It 'Should have ConfigurationId parameter as mandatory' {
            $command = Get-Command Test-TfcHYOKConfiguration
    ...
    Context 'API Interaction' {
        It 'Should call correct API endpoint with POST' {
            Test-TfcHYOKConfiguration -ConfigurationId 'hyok-abc123'
            Should -Invoke Invoke-TfcApi -ModuleName TerraformCloud -Times 1 -ParameterFilter {
                $Uri -eq '/hyok-configurations/hyok-abc123/actions/test' -and $Method -eq 'POST'
            }
        }
    }
    ...
    Context 'Verbose Output' {
        It 'Should write verbose message' {
            $verboseOutput = Test-TfcHYOKConfiguration -ConfigurationId 'hyok-abc123' -Verbose 4>&1
