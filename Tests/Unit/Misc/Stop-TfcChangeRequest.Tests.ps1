...
Describe 'Stop-TfcChangeRequest' {
    BeforeEach { Mock Invoke-TfcApi -ModuleName TerraformCloud }
    Context 'Parameter Validation' {
        It 'Should have ChangeRequestId parameter as mandatory' {
            (Get-Command Stop-TfcChangeRequest).Parameters['ChangeRequestId'].Attributes.Mandatory | Should -Be $true
        }
    }
    Context 'API Interaction' {
        It 'Should call correct API endpoint with POST' {
            Stop-TfcChangeRequest -ChangeRequestId 'cr-abc123' -Confirm:$false
            Should -Invoke Invoke-TfcApi -ModuleName TerraformCloud -Times 1 -ParameterFilter {
                $Uri -eq '/change-requests/cr-abc123/actions/cancel' -and $Method -eq 'POST'
            }
        }
    }
    Context 'ShouldProcess' {
        It 'Should not call API when WhatIf is specified' {
            Stop-TfcChangeRequest -ChangeRequestId 'cr-abc123' -WhatIf
