...
Describe 'Invoke-TfcOrganizationMembershipInvite' {
    BeforeEach {
        Mock Invoke-TfcApi -ModuleName TerraformCloud -MockWith {
            return @{ data = Get-MockOrganization }
        }
    }
    
    Context 'Function Exists' {
        It 'Should be available' {
            Get-Command Invoke-TfcOrganizationMembershipInvite | Should -Not -BeNullOrEmpty
        }
    }
