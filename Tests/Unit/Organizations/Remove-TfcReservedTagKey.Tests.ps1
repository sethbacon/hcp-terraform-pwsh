...
Describe 'Remove-TfcReservedTagKey' {
    BeforeEach { Mock Invoke-TfcApi -ModuleName TerraformCloud }
    Context 'Parameter Validation' {
        It 'Should have ReservedTagId parameter as mandatory' {
            (Get-Command Remove-TfcReservedTagKey).Parameters['ReservedTagId'].Attributes.Mandatory | Should -Be $true
        }
    }
    Context 'API Interaction' {
        It 'Should call correct API endpoint with DELETE' {
            Remove-TfcReservedTagKey -ReservedTagId 'rtag-abc123' -Confirm:$false
            Should -Invoke Invoke-TfcApi -ModuleName TerraformCloud -Times 1 -ParameterFilter {
                $Uri -eq '/reserved-tags/rtag-abc123' -and $Method -eq 'DELETE'
            }
        }
    }
    Context 'ShouldProcess' {
        It 'Should not call API when WhatIf is specified' {
            Remove-TfcReservedTagKey -ReservedTagId 'rtag-abc123' -WhatIf
