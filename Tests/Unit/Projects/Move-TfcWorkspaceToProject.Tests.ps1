...
Describe 'Move-TfcWorkspaceToProject' {
    ...
    Context 'API Interaction' {
        It 'Should call correct API endpoint with POST' {
            Move-TfcWorkspaceToProject -ProjectId 'prj-abc123' -WorkspaceIds @('ws-abc123') -Confirm:$false
            Should -Invoke Invoke-TfcApi -ModuleName TerraformCloud -Times 1 -ParameterFilter {
                $Uri -eq '/projects/prj-abc123/relationships/workspaces' -and $Method -eq 'POST'
            }
        }
    }
    ...
    Context 'ShouldProcess' {
        It 'Should not call API when WhatIf is specified' {
            Move-TfcWorkspaceToProject -ProjectId 'prj-abc123' -WorkspaceIds @('ws-abc123') -WhatIf
