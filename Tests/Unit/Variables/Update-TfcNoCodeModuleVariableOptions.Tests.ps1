...
Describe 'Update-TfcNoCodeModuleVariableOptions' {
    BeforeEach {
        Mock Invoke-TfcApi -ModuleName TerraformCloud -MockWith {
            return @{ data = Get-MockWorkspaceVariable }
        }
    }
    
    Context 'Function Exists' {
        It 'Should be available' {
            Get-Command Update-TfcNoCodeModuleVariableOptions | Should -Not -BeNullOrEmpty
        }
    }
